...
async def create_app_configuration(
    ...
    if crud.app_configuration_exists(db_session, db_project.id, db_app.id):
        raise HTTPException(
    ...
    db_app_configuration = crud.create_app_configuration(
        db_session,
        db_project.id,
        db_app.id,
...
async def list_app_configurations(
    ...
    db_project = crud.get_project_by_api_key_id(db_session, api_key_id)
    return crud.get_app_configurations(db_session, db_project.id, app_id=app_id)
...
async def get_app_configuration(
    ...
    db_project = crud.get_project_by_api_key_id(db_session, api_key_id)
    db_app_configuration = crud.get_app_configuration(db_session, db_project.id, app_id)
...
async def delete_app_configuration(
    ...
    crud.delete_linked_accounts(db_session, db_project.id, app_id)
    # 2. Delete the app configuration record
    crud.delete_app_configuration(db_session, db_project.id, app_id)
    db_session.commit()
