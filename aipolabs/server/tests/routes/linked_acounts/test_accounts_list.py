...
def test_list_linked_accounts_no_filters(
    ...
    dummy_github_linked_account_under_dummy_project_1: LinkedAccount,
) -> None:
...
def test_list_linked_accounts_filter_by_app_id(
    ...
    response = test_client.get(
        f"{config.ROUTER_PREFIX_LINKED_ACCOUNTS}/",
        headers={"x-api-key": dummy_api_key_1},
        params={"app_id": str(dummy_google_linked_account_under_dummy_project_1.app_id)},
    )
    assert response.status_code == status.HTTP_200_OK, response.json()
    assert (
        len(response.json()) == 1
    ), "Should only return linked accounts of google app under dummy project 1"
    assert response.json()[0]["app_id"] == str(
        dummy_google_linked_account_under_dummy_project_1.app_id
    )
    assert (
        response.json()[0]["linked_account_owner_id"]
        == dummy_google_linked_account_under_dummy_project_1.linked_account_owner_id
    )
...
def test_list_linked_accounts_filter_by_account_name(
    ...
    response = test_client.get(
        ...
        headers={"x-api-key": dummy_api_key_1},
        params={
            "linked_account_owner_id": dummy_github_linked_account_under_dummy_project_1.linked_account_owner_id
        },
    )
    assert response.status_code == status.HTTP_200_OK, response.json()
    assert (
    ...
    assert response.json()[0]["app_id"] == str(
        dummy_github_linked_account_under_dummy_project_1.app_id
    )
    assert (
        response.json()[0]["linked_account_owner_id"]
        == dummy_github_linked_account_under_dummy_project_1.linked_account_owner_id
    )
...
def test_list_linked_accounts_filter_by_app_id_and_account_owner_id(
    ...
    response = test_client.get(
        ...
        params={
            "app_id": str(dummy_google_linked_account_under_dummy_project_2.app_id),
            "linked_account_owner_id": dummy_google_linked_account_under_dummy_project_2.linked_account_owner_id,
        },
    )
    assert response.status_code == status.HTTP_200_OK, response.json()
    assert len(response.json()) == 1
    assert response.json()[0]["app_id"] == str(
        dummy_google_linked_account_under_dummy_project_2.app_id
    )
    assert (
        response.json()[0]["linked_account_owner_id"]
        == dummy_google_linked_account_under_dummy_project_2.linked_account_owner_id
    )
